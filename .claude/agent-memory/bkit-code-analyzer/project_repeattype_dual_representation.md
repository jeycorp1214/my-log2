...
`logs.repeatType` 컬럼은 "반복 없음"을 **두 가지 값**으로 표현한다: `NULL`(app/logs/new.tsx의 신규 저장 경로, `undefined`→NULL)과 `'none'` 문자열(app/logs/[id].tsx의 수정 경로 및 state 초기값).

**Why:** 신규/수정 저장 경로가 정규화되지 않았고, 일부 데이터가 'none' 문자열로 DB에 남는다. local-first 앱이라 구버전 데이터 마이그레이션이 ensureColumns 안전망에만 의존.

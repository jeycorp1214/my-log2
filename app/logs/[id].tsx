...
export default function LogDetailScreen() {
    ...
    if (editingMode === "copy") {
      ...
      if (selectedPersonIds.length > 0) {
        await db.insert(logPersons).values(
          selectedPersonIds.map((personId) => ({ logId: newLog.id, personId })),
        );
      }
      router.back();
    } else {
  ...
  const displayDate = isOccurrenceView && !isEditing ? occurrenceDateObj! : new Date(log.logDate);
    ...
    >
      {isEditing ? (
        <>
          {isCopyMode && (
            <View className="bg-app-teal-dark rounded-[10px] px-3 py-2 mb-1">
              <Text className="text-app-teal text-[13px]">이 날짜만 별도 기록으로 저장됩니다</Text>
            </View>
          )}
          ...
          <Pressable onPress={() => setEditingMode("none")} className="items-center py-3">
        ...
        <>
          {isOccurrenceView && (
            <View className="bg-app-teal-dark rounded-[10px] px-3 py-2 mb-1">
              <Text className="text-app-teal text-[13px]">🔄 반복 기록 — {formatLogDate(occurrenceDateObj!)}</Text>
            </View>
          )}

          <View className="flex-row items-start justify-between">
            <Text className="flex-1 text-white text-[22px] font-bold mr-3">{log.title}</Text>
            <Pressable onPress={handleEditPress} className="bg-app-surface rounded-lg px-3 py-1.5">
              <Text className="text-app-teal text-[14px]">수정</Text>
            </Pressable>
          </View>
          <Text className="text-app-muted text-[13px]">{formatLogDate(displayDate)}</Text>
          {log.memo ? (
            <Text className="text-app-label text-[15px] leading-[22px] mt-2">{log.memo}</Text>
          ) : null}
